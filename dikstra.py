...
# 다익스트라, 플로이드 워셜, 벨만 포드 알고리즘  이렇게 3가지
...
def dijkstra(start):
    ...
    distance[start] = 0
    visited[start] = True
    # 지금 그래프가 [[], [(2, 2), (3, 5), (4, 1)], [(3, 3),.....]]이런식으로 가는거임
...
# 개선된 다익스트라 알고리즘
# 여기서는 힙(Heap) 자료구조를 이용하게 된다. 특정 노드까지의 최단 거리에 대한 정보를 힙에 담아서
# 출발 노드로부터 가장 거리가 짧은 노드를 더욱 바르게 찾을 있다.
...
# 실질적으로 여기서 부터 알고리즘 시작하는거임 위에는 별 상관 X
